...
- **`ref_fasta_index`** *(guarded so it's only defined once even though an identical copy exists in `phastcons_cnees.smk`)* - Runs `samtools faidx` on the reference FASTA to produce its `.fai` index.
...
- **`make_group_beds`** - Runs `make_group_beds.py` against the reference FASTA index to write a BED file listing the chromosome coordinates belonging to one `chromosome_group`.
...
- **`ref_fasta_index`** *(guarded, duplicate of the copy in `phylofit_models.smk`)* - Runs `samtools faidx` on the reference FASTA.

- **`ref_fasta_dict`** *(guarded)* - Runs `picard CreateSequenceDictionary` on the reference FASTA, producing the `.dict` file Picard tools require.
...
- **`maf_split_chunks`** - Runs `mafutils fetch -m block` to pull MAF sub-blocks out for each chunk-boundary BED interval (N-run or fixed-window, whichever strategy is active), and writes a manifest of the chunk MAF files produced.
...
- **`phastcons_chunk`** - Runs `phastCons --rho <rho> --most-conserved` on a single chunk, with its own skip logic for invalid/oversized rho. **Orphaned**: its output path is identical to what `phastcons_apply_rho_chr` already writes per-chunk internally, nothing references `rules.phastcons_chunk...` anywhere, and it isn't part of any target list - Snakemake never actually schedules this rule either.

## Notes for whoever reads this next

- **Two orphaned rules exist**: `phastcons_estimate_rho_chunk` and `phastcons_chunk` are fully defined but never reachable from any target - their real work happens inline inside `global_rho` and `phastcons_apply_rho_chr` instead. They still get read/parsed by Snakemake (and their names are borrowed as the shared `rule_resources` key for the real per-chunk-ish rules), but never scheduled as jobs. Worth deleting or wiring back in - a separate decision from the `cnee_mafs_chr` -> `cnee_alignments_chr` rename made above.
...
- `ref_fasta_index` (and `ref_fasta_dict`) are defined identically in both files, each guarded by a `config["__..._rule_defined__"]` flag so only one copy actually registers when both workflow files are included together.
