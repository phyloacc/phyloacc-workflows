...
rule split_maf_chr:
    ...
    output:
        chromosome_maf = expand(os.path.join(MAF_SPLIT_CHR_DIR, "{{chromosome_group}}", MAF_CHR_PREFIX + "{ref_chromosome}.01.maf"), ref_chromosome=REF_CHROMOSOMES)
    params:
...
rule extract_4d_codons:
    input:
        chromosome_maf = os.path.join(MAF_SPLIT_CHR_DIR, "{chromosome_group}",  MAF_CHR_PREFIX + "{ref_chromosome}.01.maf"),
        chromosome_gff = os.path.join(OUTDIR, "gffs", "{chromosome_group}", REF_GFF_FILE.replace(".gff", "{ref_chromosome}.gff"))
    output:
