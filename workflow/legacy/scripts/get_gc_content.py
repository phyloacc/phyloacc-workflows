...
with open(output_file, "w") as out_stream:
    ...
    for line in open(sample_file):
        ...
        if first:
            ...
            accession_headers = ["accession", "ncbi.accession"];
            accession_header = False;
            for candidate_header in accession_headers:
                if candidate_header in headers:
                    accession_header = candidate_header;
                    break;

            if not accession_header:
                print("Error: none of the supported accession headers were found: " + ", ".join(accession_headers));
                sys.exit(1);
            else:
                accession_index = headers.index(accession_header);
